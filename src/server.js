...
app.post('/webhooks/livechat',async(req,res)=>{
  try{
    ...
    if(!chatId) return res.status(202).json({ok:true,ignored:true});
    ...
    if(config.lcDurableQueueEnabled){
      const eventId=String(ev?.id||p?.event_id||crypto.randomUUID());
      const createdAt=String(ev?.created_at||p?.created_at||new Date().toISOString());
      const summary=(p.chat && typeof p.chat==='object')?{...p.chat,id:chatId}:{id:chatId,webhook_event_id:eventId,webhook_created_at:createdAt};
      const summaryHash=crypto.createHash('sha1').update(JSON.stringify({chatId,eventId,createdAt,type:p?.type||p?.action||'',summary})).digest('hex');
      await enqueueLiveChatSyncJobs([{chatId,summary,summaryHash,priority:2000}]);
      return res.status(202).json({ok:true,queued:true});
    }
    if(!ev?.text) return res.status(202).json({ok:true,ignored:true});
