...
export const config = {
  ...
  lcTrackedBatchSize: Math.min(1000, Math.max(8, int(process.env.LIVECHAT_TRACKED_BATCH_SIZE, 64))),
  lcTrackedConcurrency: Math.min(64, Math.max(2, int(process.env.LIVECHAT_TRACKED_CONCURRENCY, 8))),
  lcDurableQueueEnabled: bool(process.env.LIVECHAT_DURABLE_QUEUE_ENABLED, true),
  lcDurableQueueLoadSize: Math.min(1000, Math.max(16, int(process.env.LIVECHAT_DURABLE_QUEUE_LOAD_SIZE, 128))),
  lcDurableQueueLeaseSeconds: Math.min(600, Math.max(30, int(process.env.LIVECHAT_DURABLE_QUEUE_LEASE_SECONDS, 120))),
