...
export async function migrate() {
  ...
  CREATE TABLE IF NOT EXISTS livechat_sync_jobs (
  ...
  CREATE INDEX IF NOT EXISTS idx_livechat_sync_jobs_ready ON livechat_sync_jobs(status,next_attempt_at,priority DESC,updated_at);
  CREATE INDEX IF NOT EXISTS idx_livechat_sync_jobs_locked ON livechat_sync_jobs(status,locked_at) WHERE status='PROCESSING';
...
export async function enqueueLiveChatSyncJobs(items=[]){
  const rows=[];
  for(const item of Array.isArray(items)?items:[]){
    const chatId=String(item?.chatId||item?.chat_id||item?.summary?.id||'').trim();
    if(!chatId)continue;
    rows.push({
      chat_id:chatId,
      summary:(item?.summary && typeof item.summary==='object')?item.summary:{id:chatId},
      summary_hash:String(item?.summaryHash||item?.summary_hash||''),
  ...
  const r=await pool.query(`
    WITH incoming AS (
      ...
      AS x(chat_id text,summary jsonb,summary_hash text,priority int)
    )
    INSERT INTO livechat_sync_jobs(chat_id,summary,summary_hash,priority,status,generation,attempts,next_attempt_at,created_at,updated_at)
    SELECT chat_id,COALESCE(summary,'{}'::jsonb),COALESCE(summary_hash,''),COALESCE(priority,100),'PENDING',1,0,now(),now(),now()
    FROM incoming
    ON CONFLICT(chat_id) DO UPDATE SET
      summary=EXCLUDED.summary,
      priority=GREATEST(livechat_sync_jobs.priority,EXCLUDED.priority),
      generation=CASE WHEN livechat_sync_jobs.summary_hash IS DISTINCT FROM EXCLUDED.summary_hash THEN livechat_sync_jobs.generation+1 ELSE livechat_sync_jobs.generation END,
      status=CASE
        WHEN livechat_sync_jobs.summary_hash IS DISTINCT FROM EXCLUDED.summary_hash THEN 'PENDING'
        WHEN livechat_sync_jobs.status='FAILED' THEN 'PENDING'
        ELSE livechat_sync_jobs.status
      END,
      next_attempt_at=CASE WHEN livechat_sync_jobs.summary_hash IS DISTINCT FROM EXCLUDED.summary_hash THEN now() ELSE livechat_sync_jobs.next_attempt_at END,
      locked_at=CASE WHEN livechat_sync_jobs.summary_hash IS DISTINCT FROM EXCLUDED.summary_hash THEN NULL ELSE livechat_sync_jobs.locked_at END,
      completed_at=CASE WHEN livechat_sync_jobs.summary_hash IS DISTINCT FROM EXCLUDED.summary_hash THEN NULL ELSE livechat_sync_jobs.completed_at END,
      last_error=CASE WHEN livechat_sync_jobs.summary_hash IS DISTINCT FROM EXCLUDED.summary_hash THEN NULL ELSE livechat_sync_jobs.last_error END,
      summary_hash=EXCLUDED.summary_hash,
      updated_at=now()
    RETURNING chat_id`,[JSON.stringify(rows)]);
  return r.rowCount;
}

export async function claimLiveChatSyncJobs(limit=128,{leaseSeconds=120}={}){
  const n=Math.max(1,Math.min(Number(limit)||128,1000));
  const lease=Math.max(30,Math.min(Number(leaseSeconds)||120,600));
  const r=await pool.query(`
    WITH picked AS (
      SELECT chat_id FROM livechat_sync_jobs
      WHERE (status='PENDING' AND next_attempt_at<=now())
         OR (status='PROCESSING' AND locked_at<now()-($2::text||' seconds')::interval)
      ORDER BY priority DESC,updated_at ASC,chat_id ASC
      LIMIT $1
      FOR UPDATE SKIP LOCKED
    )
    UPDATE livechat_sync_jobs j
    SET status='PROCESSING',locked_at=now(),attempts=j.attempts+1,updated_at=now()
    FROM picked p
    WHERE j.chat_id=p.chat_id
    RETURNING j.chat_id,j.summary,j.summary_hash,j.priority,j.generation,j.attempts`,[n,String(lease)]);
  return r.rows;
}

export async function completeLiveChatSyncJob(chatId,{generation=null,summaryHash=null}={}){
  const id=String(chatId||'').trim(); if(!id)return false;
  const params=[id]; let guard='';
  if(generation!=null){params.push(Number(generation));guard=` AND generation=$${params.length}`;}
  else if(summaryHash!=null){params.push(String(summaryHash));guard=` AND summary_hash=$${params.length}`;}
  const r=await pool.query(`UPDATE livechat_sync_jobs SET status='DONE',locked_at=NULL,completed_at=now(),last_error=NULL,updated_at=now() WHERE chat_id=$1${guard}` ,params);
  return r.rowCount>0;
}

export async function retryLiveChatSyncJob(chatId,{generation=null,error='',delayMs=1000}={}){
  const id=String(chatId||'').trim(); if(!id)return false;
  const delay=Math.max(250,Math.min(Number(delayMs)||1000,300000));
  const params=[id,String(error||'').slice(0,2000),delay];
  let guard='';
  if(generation!=null){params.push(Number(generation));guard=` AND generation=$${params.length}`;}
  const r=await pool.query(`UPDATE livechat_sync_jobs SET status='PENDING',locked_at=NULL,last_error=$2,next_attempt_at=now()+($3::text||' milliseconds')::interval,updated_at=now() WHERE chat_id=$1${guard}`,params);
  return r.rowCount>0;
}

export async function getLiveChatSyncQueueStats(){
  const r=await pool.query(`SELECT status,count(*)::int AS n FROM livechat_sync_jobs GROUP BY status`);
  const out={PENDING:0,PROCESSING:0,DONE:0,FAILED:0};
  for(const row of r.rows)out[String(row.status||'PENDING')]=Number(row.n||0);
  return out;
}
