...
export async function processCustomerMessage({chatId,eventId,threadId=null,text,createdAt,livechat,attachments=[],skipInsert=false,resumeReason=''}) {
  ...
  if(!skipInsert){
    ...
    const claim=await db.claimCustomerEventProcessing(chatId,eventId,{threadId,staleSeconds:config.lcIngressStaleSeconds});
    if(!claim.claimed){
  ...
  try{
    const processingResult=await db.withChatLock(chatId, async()=>{
